...
ID3D12Device* Stage1uraBlockObject::device = nullptr;
ID3D12GraphicsCommandList* Stage1uraBlockObject::cmdList = nullptr;
ComPtr<ID3D12RootSignature> Stage1uraBlockObject::rootsignature;
ComPtr<ID3D12PipelineState> Stage1uraBlockObject::pipelinestate;


void Stage1uraBlockObject::StaticInitialize(ID3D12Device* device)
{
	Stage1uraBlockObject::device = device;
}

void Stage1uraBlockObject::InitializeGraphicsPipeline()
{
	HRESULT result;
...
void Stage1uraBlockObject::InitializeDescriptorHeap()
{
	HRESULT result;
...
void Stage1uraBlockObject::InitializeCamera(int window_width, int window_height)
{
	// ビュー行列の生成
...
Stage1uraBlockObject* Stage1uraBlockObject::Create(int window_width, int window_height)
{
	// 3Dオブジェクトのインスタンスを生成
	Stage1uraBlockObject* blockObject = new Stage1uraBlockObject();

	blockObject->Initialize(window_width, window_height);

	return blockObject;
}

Stage1uraBlockObject::~Stage1uraBlockObject()
{
	if (collider)
	{
		//コリジョンマネージャから登録を解除する
...
void Stage1uraBlockObject::Initialize(int window_width, int window_height)
{
	// デスクリプタヒープの初期化
...
void Stage1uraBlockObject::CreateModel()
{
	HRESULT result;
...
void Stage1uraBlockObject::Update()
{
	HRESULT result;
...
void Stage1uraBlockObject::PreDraw(ID3D12GraphicsCommandList* cmdList)
{
	// コマンドリストをセット
	Stage1uraBlockObject::cmdList = cmdList;
...
void Stage1uraBlockObject::Draw()
{
	//頂点バッファの設定コマンド
...
void Stage1uraBlockObject::SetCollider(BaseCollider* collider)
{
	collider->SetObject(parent2);
...
void Stage1uraBlockObject::PostDraw()
{
	Stage1uraBlockObject::cmdList = nullptr;
}

void Stage1uraBlockObject::LoadMaterial(const std::string& directoryPath, const std::string& filename)
{
	//ファイルストリーム
	ifstream file;
	//マテリアルファイルを開く
...
bool Stage1uraBlockObject::LoadTexture(const std::string& directoryPath, const std::string& filename)
{
